...
- (void)viewDidLoad {
    ...
    self.myTextView.text = [NSString stringWithFormat:@"Procedure to Restore Backup:\n\n\nStep 1: Connect your iPhone to Mac via USB.\n\nStep 2: Launch iTunes Click on Devices -> Apps -> File Sharing -> All Passwords.\n\nStep 3: Click on  Add button and add the allpassword.backup file then click Restore button from your iPhone."];
    
    NSMutableAttributedString * string = [[NSMutableAttributedString alloc] initWithString:self.myTextView.text];
    
    [string addAttribute:NSForegroundColorAttributeName value:[UIColor grayColor] range:NSMakeRange(0,31)];
    [string addAttribute:NSForegroundColorAttributeName value:[UIColor grayColor] range:NSMakeRange(31,7)];
    [string addAttribute:NSForegroundColorAttributeName value:[UIColor grayColor] range:NSMakeRange(74,9)];
    [string addAttribute:NSForegroundColorAttributeName value:[UIColor grayColor] range:NSMakeRange(156,10)];
    [string addAttribute:NSForegroundColorAttributeName value:[UIColor grayColor] range:NSMakeRange(233,8)];
    self.myTextView.attributedText = string;
    [self.myTextView setFont:[UIFont boldSystemFontOfSize:16]];
}
