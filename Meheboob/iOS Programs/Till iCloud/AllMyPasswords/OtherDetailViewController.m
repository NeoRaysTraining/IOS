...
@interface OtherDetailViewController ()<MFMailComposeViewControllerDelegate,MFMessageComposeViewControllerDelegate,UIScrollViewDelegate>{
    ...
    int getIndexpath;
    OtherDetailTableViewCell *textFieldCellCheck;
    ...
    NSMutableDictionary *textFieldArray1;
    NSMutableArray *textFieldArray2;
    
    
    BOOL textFieldFlag;
    BOOL textFieldEditFlag;
...
- (void)viewDidLoad {
    [super viewDidLoad];
    textFieldFlag = NO;
   //textFieldArray1 = [[NSMutableDictionary alloc]init];
 //   textFieldArray2 = [[NSMutableArray alloc]init];
    textFieldArray1 = [[NSMutableDictionary alloc] init];
    
    //[textFieldArray1 setObject:@"This object" forKey:@"1234"];
   // textFieldCellCheck = [[OtherDetailTableViewCell alloc] init];
    ...
    for (int i = 0; i<reciedKeyArray.count; i++) {
          [textFieldArray1 setObject:[reciedKeyArray objectAtIndex:i] forKey:[NSNumber numberWithInt:i]];
    }
    ...
    for (int i = 0; i<reciedValArray.count; i++) {
        [textFieldArray1 setObject:[reciedValArray objectAtIndex:i] forKey:[NSNumber numberWithInt:i+100]];
    }
...
{
    int count = (int)[textFieldArray1 count]/2;
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if(indexPath.row == ([textFieldArray1 count]/2) && self.editing){
        cell.textField1.text = @"Add new row";
        cell.textField2.text = @"";
          cell.textField1.tag = indexPath.row;
        cell.textField2.tag = (int)indexPath.row+100;
    ...
    cell.textField1.text = [textFieldArray1 objectForKey:[NSNumber numberWithInt:(int)indexPath.row]];
    cell.textField1.tag = (int)indexPath.row;
  
    cell.textField2.text = [textFieldArray1 objectForKey:[NSNumber numberWithInt:(int)indexPath.row+100]];
    cell.textField2.tag = (int)indexPath.row+100;
...
- (UITableViewCellEditingStyle)tableView:(UITableView *)aTableView editingStyleForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (self.editing && indexPath.row == ([textFieldArray1 count]/2))
...
- (void)tableView:(UITableView *)tableView commitEditingStyle:(UITableViewCellEditingStyle) editingStyle forRowAtIndexPath:(NSIndexPath *)indexPath
{
    if (editingStyle == UITableViewCellEditingStyleDelete)
    {
       ...
       // [reciedValArray removeObjectAtIndex:indexPath.row];
        ...
        [textFieldArray1 removeObjectForKey:[NSNumber numberWithInt:(int)indexPath.row]];
        [textFieldArray1 removeObjectForKey:[NSNumber numberWithInt:(int)indexPath.row+100]];
        [self.myTableView reloadData];
    }
    else if (editingStyle == UITableViewCellEditingStyleInsert)
    {
       // [reciedKeyArray insertObject:@"New Key" atIndex:[reciedKeyArray count]];
        // [reciedValArray insertObject:@"New Val" atIndex:[reciedValArray count]];
         ...
         [textFieldArray1 setObject:@"New Key" forKey:[NSNumber numberWithInteger:(int)indexPath.row]];
        [textFieldArray1 setObject:@"New Value" forKey:[NSNumber numberWithInteger:(int)indexPath.row+100]];
...
//    UIView *tableViewCellView = [[self.myTableView superview] superview];
...
//    }
    ...
    for (int i=0; i < textFieldArray1.count/2; i++){
...
//        
//        NSString *textFiedlKey = [[NSString alloc] initWithFormat:@"%@",[cellTextField text]];
        NSString *textFiedlKey = [NSString stringWithFormat:@"%@",[textFieldArray1 objectForKey:[NSNumber numberWithInt:i]]];
...
//        
//        NSString *textFiedlVal = [[NSString alloc] initWithFormat:@"%@",[cellTextField text]];[cellTextField text];
        
      NSString *textFiedlKey = [NSString stringWithFormat:@"%@",[textFieldArray1 objectForKey:[NSNumber numberWithInt:i+100]]];
        [editedValArray addObject:textFiedlKey];
    }
...
-(BOOL)textFieldShouldReturn:(UITextField *)textField{
    ...
    {
        UITableViewCell *cell2 = (UITableViewCell*)textField.superview.superview;
        NSIndexPath *indexPath = [self.myTableView indexPathForCell:cell2];
        
        [self.myTableView scrollToRowAtIndexPath:indexPath atScrollPosition:UITableViewScrollPositionMiddle animated:TRUE];
    }
    return YES;
}
...
- (void)textFieldDidEndEditing:(UITextField *)textField{
    ...
    [textFieldArray1 removeObjectForKey:[NSNumber numberWithInt:(int)textField.tag]];
 
        [textFieldArray1 setObject:textField.text forKey:[NSNumber numberWithInteger:textField.tag]];
   
    
}

-(BOOL)textFieldShouldBeginEditing:(UITextField *)textField {
    CGPoint pointInTable = [textField.superview convertPoint:textField.frame.origin toView:self.myTableView];
    ...
    [self.myTableView setContentOffset:contentOffset animated:YES];
